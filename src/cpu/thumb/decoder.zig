...
fn decodeLSRegOffsetTInstr(instr: u16) is.LSRegOffsetTInstr {
    return .{
        .load = getNBits(instr, 11, 1, u1) == 1,
        .byte = getNBits(instr, 10, 1, u1) == 1,
        .ro = getNBits(instr, 6, 3, u3),
        .rb = getNBits(instr, 3, 3, u3),
        .rd = getNBits(instr, 0, 3, u3),
    };
}

fn decodeLSImmOffsetTInstr(instr: u16) is.LSImmOffsetTInstr {
    return .{
        .byte = getNBits(instr, 12, 1, u1) == 1,
        .load = getNBits(instr, 11, 1, u1) == 1,
        .offset = getNBits(instr, 6, 5, u5),
        .rb = getNBits(instr, 3, 3, u3),
        .rd = getNBits(instr, 0, 3, u3),
...
fn decodeLSSignExTInstr(instr: u16) is.LSSignExTInstr {
    return .{
        .opcode = @enumFromInt(getNBits(instr, 10, 2, u2)),
        .ro = getNBits(instr, 6, 3, u3),
        .rb = getNBits(instr, 3, 3, u3),
        .rd = getNBits(instr, 0, 3, u3),
    };
...
fn decodeLongBranchWithLinkTInstr(instr: u16) is.LongBranchWithLinkTInstr {
    return .{
        .prefix = getNBits(instr, 11, 1, u1) == 1,
        .offset = getNBits(instr, 0, 11, u11),
    };
}
...
pub fn decode(instr: u16) InstrDecodeError!is.ThumbInstr {
    ...
    const ls_reg_offset_bitmask = 0b1111_00_1_000000000;
    const ls_reg_offset_test = 0b0101_00_0_000000000;
    ...
    const multiple_ls_bitmask = 0b1111_000000000000;
    const multiple_ls_test = 0b1100_000000000000;

    const long_branch_with_link_bitmask = 0b1111_000000000000;
    const long_branch_with_link_test = 0b1111_000000000000;
    ...
    else if (instr & ls_sign_ex_bitmask == ls_sign_ex_test)
        decoded_instr = .{ .ls_sign_ex = decodeLSSignExTInstr(instr) }
    ...
    else if (instr & long_branch_with_link_bitmask == long_branch_with_link_test)
        decoded_instr = .{ .long_branch_with_link = decodeLongBranchWithLinkTInstr(instr) }
